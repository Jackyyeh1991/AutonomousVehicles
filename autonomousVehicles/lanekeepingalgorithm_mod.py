...
target = 15 #Target value we want to get from our encoder 
...
while counter < max_ticks:
    ...
    # speed adjustment
    if counter%5 == 0 and not atStopLight:
        ...
        print(speed)
        if speed > target:
            go_forward = go_forward - 0.05
        elif speed < target:
            go_forward = go_forward + 0.05
        if go_forward > 8.45:
            go_forward = 8.45 #limit the highest speed
        if go_forward < 8.1:
            go_forward = 8.1
        throttle (go_forward)
